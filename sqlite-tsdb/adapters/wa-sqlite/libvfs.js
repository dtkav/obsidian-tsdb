// Vendored from wa-sqlite src/libvfs.js at commit
// a110948636473279dd3590f0b980bc9c6a9d6407, with 64-bit file offsets read
// correctly. The C bridge (libvfs.c) passes sqlite3_int64 offsets and sizes
// by pointer, and Emscripten's getValue(ptr, 'i64') without WASM_BIGINT
// loads only the low 32 bits as a signed integer. Past 2 GiB that hands the
// JavaScript VFS a wrapped, negative offset, so reads and writes land on the
// wrong pages and SQLite walks garbage. u64() combines both 32-bit halves.
// This mirrors upstream commit 57d7ff863540e67718b14f6bc08ce1e0e1363f0c.
...
const vfs_methods = {
  ...
  $vfs_method_support: function() {
    ...
    // Convert a 64-bit unsigned integer in WASM memory to a Number. File
    // offsets and sizes are assumed to be <= Number.MAX_SAFE_INTEGER, so
    // the high word is scaled rather than shifted.
    function u64(ptr) {
      const index = ptr >> 2;
